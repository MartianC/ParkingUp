...
namespace libx
{
    ...
    public class Updater : MonoBehaviour, IUpdater, INetworkMonitorListener
    {
        ...
        [SerializeField] private string baseURL = "http://114.215.82.214:8088/";
        [SerializeField] private string gameScene = "Login.unity";
        [SerializeField] private bool enableVFS = false;
...
// #endif
            ...
            _savePath = string.Format("{0}/", Application.persistentDataPath);
        ...
        IEnumerator LocalCompareRes()
        {
            ...
            if (string.IsNullOrEmpty(init.error))
            {
                ...
                Assets.AddSearchPath("Assets/HotUpdateResources/UIAtlas");
                init.Release();
                OnProgress(0);
                OnMessage("加载游戏场景");
                var scene = Assets.LoadSceneAsync(gameScene, false);
                while (scene.progress < .9f /*!scene.isDone*/)
                {
                    OnProgress(scene.progress);
                    yield return null;
                }
                gameObject.SetActive(false);
                var us = FindObjectOfType<UpdateScreen>();
                if (us != null)
                {
        ...
        private IEnumerator UpdateCopy(IList<VFile> versions, string basePath)
        {
            ...
            //if (version.name.Equals(Versions.Dataname)) //取消复制单个VFS
            //{
        ...
        private void OnComplete()
        {
            ...
            //ver.tmp 与 ver 相互检测替换
            var tmpFileName = _savePath + Versions.Filename + ".tmp";
            var destFileName = _savePath + Versions.Filename;
            var tmpVersion = Versions.LoadVersion(tmpFileName);
            if (tmpVersion > version)
            {
                File.Replace(tmpFileName, destFileName, destFileName + ".back", true);
            }
            else
            {
                if (File.Exists(_savePath + Versions.Filename + ".tmp"))
                {
                    File.Delete(_savePath + Versions.Filename + ".tmp");
                }
            }
